...
public class ExerciseSheetController {
    ...
    public ResponseEntity<?> getExerciseSheetById(@PathVariable Long id) {
        try {
            ...
            ExerciseSheetResponseDto responseDto = exerciseSheetService.getExerciseSheetById(id, user.getId());
    ...
    public ResponseEntity<?> getAllExerciseSheetsByUser() {
        try {
            User user = rq.getActor();  // 로그인한 사용자 정보
            List<ExerciseSheetResponseDto> responseDtos = exerciseSheetService.getAllExerciseSheetsByUser(user.getId());
