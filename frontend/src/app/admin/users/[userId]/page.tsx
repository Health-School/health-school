...
const App: React.FC = () => {
  ...
  const [userToChangeStatus, setUserToChangeStatus] = useState<number | null>(null); // 상태 변경 대상 사용자의 ID
...
const openStatusModal = (userIdToUpdate: number) => {
  ...
  const handleChangeStatus = async () => {
      ...
      // setIsLoading(true); // 전체 페이지 로딩이 아닌, 부분 로딩 상태를 관리하고 싶다면 별도 상태 추가 가능
      ...
      try {
        ...
        const response = await fetch(url, {
          ...
          credentials: 'include',
          body: JSON.stringify({ userStatus: backendStatus }), // 백엔드 DTO 필드명 UserStatus
        });
        ...
        if (responseData.success) {
          ...
          setCurrentUserDetail((prevDetails: any) => ({
            ...prevDetails,
            userStatus: backendStatus // 백엔드 enum 값으로 UI 상태 업데이트
          }));
        } else {
  ...
  if (isLoading) {
   ...
   const getRoleInKorean = (role: string) => {
      ...
      return '일반 회원'; // 기본값은 일반 회원
    };

    // 상태(UserStatus)를 한글과 스타일로 변환하는 함수
    const getStatusStyleAndText = (status: string) => {
      ...
      return { text: '알 수 없음', className: 'bg-gray-200 text-gray-700' }; // 기본값
    };

    // 날짜 포맷팅 함수 (예: 2025-03-05)
    ...
    // 상세 페이지의 현재 날짜 표시 (예: 2025년 5월 19일 월요일)
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center mb-6">
            <a
              href="/admin/users" // 이전 페이지로 돌아가는 링크 (Next.js의 Link 컴포넌트 사용 권장)
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                ...
                <div>
                  ...
                  <div className="flex items-center mt-1 space-x-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${
                      ...
                      currentUserDetail.role === 'ADMIN' ? 'bg-purple-500/10 text-purple-500' : // 관리자 스타일 예시
            ...
            <div className="space-y-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4"> {/* gap-y-4로 세로 간격도 살짝 줬어 */}
                            <div className="flex items-center"> {/* items-center 추가 */}
                              <i className="fas fa-envelope text-gray-400 w-5 text-center mr-3"></i> {/* 아이콘 추가 */}
                              <div className="flex flex-col">
                                <span className="text-sm text-gray-500 mb-0.5">이메일</span> {/* mb-0.5로 간격 미세 조정 */}
              ...
              {/* 강사 전용 정보: 자기소개, 전문분야, 경력사항 */}
              {currentUserDetail.role === 'TRAINER' && (
                <>
                  {/* selfIntroduction, expertiseAreas, careerHistory 필드가 TrainerCoreInfoDto에 있다고 가정 */}
                  ...
                  {currentUserDetail.careerHistory && (
                    <div className="border-t border-gray-200 pt-6">
                      ...
                      <div className="space-y-4">
                        {/* careerHistory는 보통 여러 줄일 수 있으니, pre-line으로 처리하거나,
                            백엔드에서 리스트 형태로 내려준다면 map으로 돌려서 예쁘게 표시 */}
                         <p className="text-gray-700 whitespace-pre-line">
                           {currentUserDetail.careerHistory}
                         </p>
                      </div>
            ...
            <div className="w-full">
              <div className="bg-white rounded-lg shadow-sm mb-6">
                <div className="border-b border-gray-200">
                  <nav className="flex -mb-px overflow-x-auto"> {/* 탭이 많을 경우 가로 스크롤 */}
                    {/* 강사일 경우에만 보여줄 탭들 */}
                    {currentUserDetail.role === 'TRAINER' && (
                      <>
                        <button
                        ...
                        <button
                          onClick={() => setActiveTab('earnings')} // "결산 내역"으로 되어 있지만 UI상 "정산 내역" 및 "결산 요약"을 포함하는 개념으로 보임
                          className={`py-4 px-6 text-center border-b-2 font-medium text-sm ${
                            activeTab === 'earnings'
                              ? 'border-[#2ECC71] text-[#2ECC71]'
                              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                    ...
                    {/* 신고 내역 탭 - 이건 어떤 데이터를 보여줄지 정해지면 추가 */}
                    {/*
                    <button
                ...
                <div className="p-6">
                  ...
                  {activeTab === 'course-management' && currentUserDetail && currentUserDetail.role === 'TRAINER' && (
                    <CourseManagementTab
                    trainerId={userId} // 현재 보고 있는 회원의 ID가 곧 강사의 ID
                    apiBaseUrl={process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8090'}
                    userNickname={currentUserDetail.nickname}
                        />
                    )}
                  ...
                  {/* 여기에 다른 탭들의 컨텐츠도 추가 (활동 로그, 메모 등은 아직 API 없음) */}
