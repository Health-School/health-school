...
const App: React.FC = () => {
  ...
  const [detailModalSelectedStatus, setDetailModalSelectedStatus] =
    useState<string>("");
  ...
  const fetchReports = async (
    ...
    try {
      // API URL 구성
      let apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/admin/reports?page=${page}&size=${size}&sort=createdDate,desc`;
      if (status && status !== "ALL") {
        // 'ALL'은 백엔드에서 status 파라미터 없이 보내는 것으로 약속
      ...
      if (responseData && responseData.success) {
        ...
        if (result && result.content) {
          // result 객체와 result.content가 모두 존재하는지 확인
          if (result.content.length > 0) {
            console.log("첫 번째 신고 데이터:", result.content[0]); // 첫 번째 데이터 전체를 한번 보자
            console.log(
              "첫 번째 신고 데이터의 createdDate:",
              result.content[0].createdDate
            );
          } else {
            console.log(
              "API 응답: 신고 데이터가 비어 있습니다 (result.content is empty)."
            );
          }
        } else {
          console.log(
            "API 응답: result 또는 result.content가 undefined 입니다.",
            result
          );
        }
        ...
        throw new Error(
          responseData.message || "Failed to fetch reports from API"
        );
      }
    } catch (err: any) {
  ...
  const handleSendNotification = async () => {
    ...
    console.log(
      `알림 전송 시도: Report ID = ${selectedReport.id}, 메시지 = "${notificationMessage}"`
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          responseData.message || `알림 전송 실패: ${response.status}`
        );
      }
  ...
  const handleReportStatusUpdate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || `상태 변경 실패: ${response.status}`
        );
      }
      ...
      if (responseData && responseData.success) {
        ...
        const effectiveStatusFilter =
          statusFilter === "전체" ? null : statusFilter;
        fetchReports(currentPage, itemsPerPage, effectiveStatusFilter);
      } else {
  ...
  const openReportDetail = (report: any) => {
    ...
    setDetailModalSelectedStatus(
      findStatusEnumKey(report.reportStatusDescription)
    );
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <button className="relative p-2 text-gray-600 hover:text-[#2ECC71] transition-colors duration-200 cursor-pointer">
                  ...
                  <span className="absolute top-0 right-0 w-4 h-4 bg-[#E74C3C] text-white text-xs flex items-center justify-center rounded-full">
                    3
                  </span>
                </button>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {/* 백엔드에서 status 파라미터가 없거나 'ALL'일 때 전체 조회 약속 */}
                      ...
                      (
                        ...
                        <tr key={report.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${getStatusBadgeStyle(report.reportStatusDescription)}`}
                            >
          ...
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-500">
              {/* 기존 코드: 총 {filteredReports.length}건 중 {indexOfFirstItem + 1}~{Math.min(indexOfLastItem, filteredReports.length)}건 표시
                          ...
                          */}
              총 {totalReportCount}건 (현재 페이지:{" "}
              {isLoading || reportsData.length === 0 ? 0 : currentPage + 1} /{" "}
              {totalPages})
            </div>
            <div className="flex space-x-2">
              ...
              <button
                onClick={() =>
                  setCurrentPage((prev) =>
                    Math.min(totalPages > 0 ? totalPages - 1 : 0, prev + 1)
                  )
                }
                disabled={
                  isLoading || currentPage >= totalPages - 1 || totalPages === 0
                } // 로딩 중이거나 마지막 페이지거나 페이지가 없으면 비활성화
                className={`px-3 py-1 rounded-lg transition-colors duration-200 !rounded-button cursor-pointer whitespace-nowrap ${
                  isLoading || currentPage >= totalPages - 1 || totalPages === 0
                    ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                    : "bg-gray-100 text-gray-700 hover:bg-gray-200"
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1">
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${getStatusBadgeStyle(selectedReport.reportStatusDescription)}`}
                  >
                ...
                <div className="flex items-center space-x-2">
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) =>
                        setDetailModalSelectedStatus(e.target.value)
                      }
                      className="pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-[#2ECC71] focus:border-[#2ECC71] rounded-md appearance-none bg-white text-sm"
                    >
