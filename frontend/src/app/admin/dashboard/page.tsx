...
const DashboardPageContent: React.FC = () => {
  const [instructorApprovalFilter, setInstructorApprovalFilter] = useState<string>('all');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [selectedNotice, setSelectedNotice] = useState<Notice | null>(null);
  const [isNoticeModalOpen, setIsNoticeModalOpen] = useState<boolean>(false);
  const [userChartPeriod, setUserChartPeriod] = useState<'daily' | 'weekly' | 'monthly'>('weekly');

  const [metrics, setMetrics] = useState<MetricWidgetDto[]>([]);
  const [metricsLoading, setMetricsLoading] = useState<boolean>(true);
  const [metricsError, setMetricsError] = useState<string | null>(null);

  const [userGrowthData, setUserGrowthData] = useState<ChartDataDto | null>(null);
  ...
  const [userGrowthError, setUserGrowthError] = useState<string | null>(null);
    ...
    // 결제 금액 추이 차트 데이터
  const [salesAmountData, setSalesAmountData] = useState<ChartDataDto | null>(null);
  ...
  const [salesChartPeriod, setSalesChartPeriod] = useState<'daily' | 'weekly' | 'monthly'>('daily');
...
사용자들은 이 모달을 통해 공지사항의 전체 내용을 확인할 수 있습니다.`
  ...
  useEffect(() => {
      const fetchMetrics = async () => {
        ...
        try {
          ...
          const response = await fetch(`${apiBaseUrl}/api/v1/admin/dashboard/key-metrics`, {
            method: 'GET',
            credentials: 'include',
            headers: {
          ...
          if (!response.ok) {
            if (response.status === 403) { // 403 Forbidden (권한 없음) 에러 처리
              throw new Error('지표 데이터를 불러올 권한이 없습니다. 관리자 계정으로 로그인해주세요.');
            }

            const errorData = await response.json().catch(() => ({ message: `서버 응답 에러! 상태 코드: ${response.status}` }));
            throw new Error(errorData.message || `데이터를 불러오는데 실패했습니다. 상태 코드: ${response.status}`);
          }


          const result: { success: boolean, data: MetricWidgetDto[], message: string } = await response.json();
          ...
          if (result.success && Array.isArray(result.data)) {
            ...
            throw new Error(result.message || '데이터 형식이 올바르지 않거나 데이터가 없습니다.');
          }
        } catch (err: any) {
  ...
  useEffect(() => {
      const fetchUserGrowthData = async () => {
        ...
        try {
          const response = await fetch(`${apiBaseUrl}/api/v1/admin/dashboard/user-growth-trend?interval=${userChartPeriod}`, {
            method: 'GET',
            credentials: 'include',
          ...
          if (!response.ok) {
            if (response.status === 403) throw new Error('사용자 증가 추이 데이터를 불러올 권한이 없습니다.');
            const errorData = await response.json().catch(() => ({ message: `HTTP error! status: ${response.status}` }));
            throw new Error(errorData.message || `사용자 증가 추이 데이터 로딩 실패: ${response.status}`);
          }
          const result: { success: boolean, data: ChartDataDto, message: string } = await response.json();
          if (result.success && result.data) {
            setUserGrowthData(result.data);
          } else {
            throw new Error(result.message || '사용자 증가 추이 데이터 형식이 올바르지 않습니다.');
          }
        } catch (err: any) {
  ...
  useEffect(() => {
      const fetchSalesAmountData = async () => {
        ...
        try {
          const response = await fetch(`${apiBaseUrl}/api/v1/admin/dashboard/sales-amount-trend?interval=${salesChartPeriod}`, {
            method: 'GET',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
          });
          if (!response.ok) {
            if (response.status === 403) throw new Error('결제 금액 추이 데이터를 불러올 권한이 없습니다.');
            const errorData = await response.json().catch(() => ({ message: `HTTP error! status: ${response.status}` }));
            throw new Error(errorData.message || `결제 금액 추이 데이터 로딩 실패: ${response.status}`);
          }
          const result: { success: boolean, data: ChartDataDto, message: string } = await response.json();
          if (result.success && result.data) {
            setSalesAmountData(result.data);
          } else {
            throw new Error(result.message || '결제 금액 추이 데이터 형식이 올바르지 않습니다.');
          }
        } catch (err: any) {
  ...
  useEffect(() => {
    ...
    if (userChartElement) {
      const userChart = echarts.getInstanceByDom(userChartElement) || echarts.init(userChartElement);
      ...
      } else if (userGrowthData && userGrowthData.dataPoints && userGrowthData.dataPoints.length > 0) {
          ...
          series: [{
            ...
            areaStyle: {
              color: {
                ...
                colorStops: [{ offset: 0, color: 'rgba(46, 204, 113, 0.5)' }, { offset: 1, color: 'rgba(46, 204, 113, 0.1)' }]
              }
            }
          }]
        };
    ...
    if (paymentChartElement) {
      const paymentChart = echarts.getInstanceByDom(paymentChartElement) || echarts.init(paymentChartElement);
      ...
      } else if (salesAmountData && salesAmountData.dataPoints && salesAmountData.dataPoints.length > 0) {
    ...
    const categoryChartElement = document.getElementById('categoryDistributionChart');
    if (categoryChartElement) {
      const categoryChart = echarts.getInstanceByDom(categoryChartElement) || echarts.init(categoryChartElement);
      const categoryOption = {
        animation: false,
        tooltip: { trigger: 'item' },
        legend: { orient: 'vertical', left: 'left', textStyle: { color: '#2C3E50' } },
        series: [{
          name: '강의 카테고리', type: 'pie', radius: '70%',
          data: [
            { value: 35, name: '요가' }, { value: 25, name: '필라테스' },
            { value: 20, name: '웨이트 트레이닝' }, { value: 15, name: '홈트레이닝' },
            { value: 5, name: '기타' }
          ],
          emphasis: { itemStyle: { shadowBlur: 10, shadowOffsetX: 0, shadowColor: 'rgba(0, 0, 0, 0.5)' } },
          itemStyle: { color: function(params: any) { const colorList = ['#2ECC71', '#3498DB', '#9B59B6', '#F1C40F', '#E74C3C']; return colorList[params.dataIndex]; } }
        }]
      };
    ...
    if (reportChartElement) {
      const reportChart = echarts.getInstanceByDom(reportChartElement) || echarts.init(reportChartElement);
      const reportOption = {
        animation: false,
        tooltip: { trigger: 'item' },
        legend: { orient: 'vertical', left: 'left', textStyle: { color: '#2C3E50' } },
        series: [{
          name: '신고 유형', type: 'pie', radius: '70%',
          data: [
            { value: 40, name: '부적절한 콘텐츠' }, { value: 25, name: '허위 정보' },
            { value: 20, name: '저작권 침해' }, { value: 15, name: '기타' }
          ],
          emphasis: { itemStyle: { shadowBlur: 10, shadowOffsetX: 0, shadowColor: 'rgba(0, 0, 0, 0.5)' } },
          itemStyle: { color: function(params: any) { const colorList = ['#E74C3C', '#F39C12', '#3498DB', '#95A5A6']; return colorList[params.dataIndex]; } }
        }]
      };
    ...
    const handleResize = () => {
      if (userChartElement && echarts.getInstanceByDom(userChartElement)) echarts.getInstanceByDom(userChartElement)?.resize();
      if (categoryChartElement && echarts.getInstanceByDom(categoryChartElement)) echarts.getInstanceByDom(categoryChartElement)?.resize();
      if (paymentChartElement && echarts.getInstanceByDom(paymentChartElement)) echarts.getInstanceByDom(paymentChartElement)?.resize();
      if (reportChartElement && echarts.getInstanceByDom(reportChartElement)) echarts.getInstanceByDom(reportChartElement)?.resize();
    };
    ...
    return () => {
      ...
      if (userChartElement && echarts.getInstanceByDom(userChartElement)) echarts.getInstanceByDom(userChartElement)?.dispose();
      if (categoryChartElement && echarts.getInstanceByDom(categoryChartElement)) echarts.getInstanceByDom(categoryChartElement)?.dispose();
      if (paymentChartElement && echarts.getInstanceByDom(paymentChartElement)) echarts.getInstanceByDom(paymentChartElement)?.dispose();
      if (reportChartElement && echarts.getInstanceByDom(reportChartElement)) echarts.getInstanceByDom(reportChartElement)?.dispose();
    };

  }, [userChartPeriod, userGrowthData, userGrowthLoading, userGrowthError, salesChartPeriod, salesAmountData, salesAmountLoading, salesAmountError]);
  ...
  return (
          <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  {metricsLoading && (
                    <div className="col-span-full text-center p-4">📊 핵심 지표 로딩 중...</div>
                  )}
                  {metricsError && (
                    <div className="col-span-full text-center p-4 text-red-500">⚠️ 핵심 지표 로딩 에러: {metricsError}</div>
                  )}

                  {!metricsLoading && !metricsError && (
                    <> {/* React Fragment 시작 */}
                      {metrics.length > 0 ? metrics.map((metricItem) => {
                        const displayConfig = metricDisplayConfig[metricItem.metricName] || {
                        ...
                        return (
                          <div key={displayConfig.displayName} className="bg-white rounded-lg shadow-sm p-6">
                            <div className="flex justify-between items-start">
                              <div>
                                <p className="text-sm text-gray-500 mb-1">{displayConfig.displayName}</p>
                                <h3 className="text-2xl font-bold text-gray-800">{metricItem.currentValue.toLocaleString()}</h3>
                                {metricItem.percentageChange !== null && (
                                  <p className={`text-xs ${metricItem.percentageChange >= 0 ? 'text-[#2ECC71]' : 'text-red-500'} mt-1`}>
                                    <i className={`fas ${metricItem.percentageChange >= 0 ? 'fa-arrow-up' : 'fa-arrow-down'} mr-1`}></i>
                                    <span>{Math.abs(metricItem.percentageChange)}% {metricItem.percentageChange >= 0 ? '증가' : '감소'}</span>
                                  </p>
                                )}
                              </div>
                              <div className={`w-12 h-12 ${displayConfig.bgColor} rounded-full flex items-center justify-center`}>
                                <i className={`${displayConfig.icon} ${displayConfig.textColor} text-xl`}></i>
                              </div>
                            </div>
                          </div>
                        );
                        ...
                        <div className="col-span-full text-center p-4">표시할 핵심 지표 데이터가 없습니다. (신고 건수 제외)</div>
                      )}
