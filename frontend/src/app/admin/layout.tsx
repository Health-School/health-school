...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  const getHeaderTitle = () => {
    ...
    if (activeMenuItem.startsWith("instructor-")) {
      ...
      if (activeMenuItem === "instructor-approval")
        return "강사 자격 신청 관리";
      if (activeMenuItem === "instructor-certification")
        return "강사 자격증 관리";
      return "강사 관리";
    }
  ...
  const handleInstructorMenuToggle = () => {
    if (
      ...
      (activeMenuItem.startsWith("instructor-") ||
        activeMenuItem === "instructor")
    ) {
      setIsInstructorMenuOpen(false);
    } else {
      ...
      <div className="w-64 bg-[#2C3E50] text-white h-full flex flex-col">
              ...
              {isInstructorMenuOpen && (
                <ul className="mt-2 space-y-1">
                  ...
                  <li>
                    <button
                      onClick={() =>
                        setActiveMenuItem("instructor-certification")
                      }
        ...
        <div className="p-4 border-t border-[#34495E]">
          <button
            ...
            className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-[#E74C3C] text-white rounded-lg hover:bg-[#C0392B] transition-colors duration-200 !rounded-button cursor-pointer whitespace-nowrap"
          >
